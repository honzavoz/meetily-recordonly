...
function rustConstant(registration, name) {
...
function verifyChromeStoreIdentity() {
  ...
  const configuredId = rustConstant(registration, 'EXTENSION_ID');
  const configuredOrigin = rustConstant(registration, 'EXTENSION_ORIGIN');
  const configuredStoreUrl = rustConstant(registration, 'CHROME_WEB_STORE_URL');
  ...
  return { extensionId: derivedId, manifest, storeUrl: configuredStoreUrl };
}

if (require.main === module) {
  const { extensionId } = verifyChromeStoreIdentity();
  console.log(`Chrome extension identity verified: ${extensionId}`);
}

module.exports = { extensionIdFromKey, verifyChromeStoreIdentity };
